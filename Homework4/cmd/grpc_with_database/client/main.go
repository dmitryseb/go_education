...
func (c *Command) create() error {
	...
	if err != nil {
		return fmt.Errorf("create grpc_with_database client failed: %w", err)
	}
	defer func() {
		_ = conn.Close()

	}()
	con := proto1.NewBankClient(conn)

	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()

	res, err := con.CreateAccountRequest(ctx, &proto1.CreateAccount{Name: c.Name, Balance: int64(c.Amount)})
	if err != nil {
		return fmt.Errorf("create account failed: %w", err)
	}
...
func (c *Command) get() error {
	conn, err := grpc.NewClient(c.Host+":"+strconv.Itoa(c.Port), grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		return fmt.Errorf("get grpc_with_database client failed: %w", err)
	}
	defer func() {
		_ = conn.Close()
	...
	res, err := con.GetAccountResponse(ctx, &proto1.GetName{Name: c.Name})
	if err != nil {
		return fmt.Errorf("get account failed: %w", err)
	}
	if res.Status != "ok" {
...
func (c *Command) delete() error {
	conn, err := grpc.NewClient(c.Host+":"+strconv.Itoa(c.Port), grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		return fmt.Errorf("delete grpc_with_database client failed: %w", err)
	}
	defer func() {
		_ = conn.Close()
	...
	res, err := con.DeleteAccountRequest(ctx, &proto1.DeleteAccount{Name: c.Name})
	if err != nil {
		return fmt.Errorf("delete account failed: %w", err)
	}
	fmt.Println("status:", res.Status)
...
func (c *Command) changeAccount() error {
	conn, err := grpc.NewClient(c.Host+":"+strconv.Itoa(c.Port), grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		return fmt.Errorf("change grpc_with_database client failed: %w", err)
	}
	defer func() {
		_ = conn.Close()
	...
	res, err := con.ChangeAccountRequest(ctx, &proto1.ChangeAccount{Name: c.Name, NewName: c.NewName})
	if err != nil {
		return fmt.Errorf("change account failed: %w", err)
	}
	fmt.Println("status:", res.Status)
...
func (c *Command) pathAccount() error {
	conn, err := grpc.NewClient(c.Host+":"+strconv.Itoa(c.Port), grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		return fmt.Errorf("path grpc_with_database client failed: %w", err)
	}
	defer func() {
		_ = conn.Close()
